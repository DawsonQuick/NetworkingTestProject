...
private:
	...
	float lifetime;
	glm::vec4 color;
	glm::mat4x2 texturePos = TextureManager::getInstance().getTextureInfo(typeid(Particle_D));
...
public:
	...
	Particle_D(float posX, float posY, float scale, glm::vec4 tmpColor) : posX(posX), posY(posY), color(tmpColor) {
		velX = 0.0;
		velY = 0.001;
