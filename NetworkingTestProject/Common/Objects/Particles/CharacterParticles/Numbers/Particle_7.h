...
#define PARTICLE_7_H
#include "./../../Particle.h"
class Particle_7 : public Particle {
private:
	float posX;
	float posY;
	float velX;
	...
	glm::vec4 color;
	glm::mat4x2 texturePos = TextureManager::getInstance().getTextureInfo(typeid(Particle_7));
...
public:
	...
	Particle_7(float posX, float posY, float scale, glm::vec4 tmpColor) : posX(posX), posY(posY), color(tmpColor) {
		velX = 0.0;
		velY = 0.001;
