...
#define PARTICLE_1_H
#include "./../../Particle.h"
class Particle_1 : public Particle {
private:
	float posX;
	float posY;
	float velX;
	...
	glm::mat4x2 texturePos = TextureManager::getInstance().getTextureInfo(typeid(Particle_1));
...
public:
	...
	Particle_1(float posX, float posY, float scale, glm::vec4 tmpColor) : posX(posX), posY(posY), color(tmpColor) {
		velX = 0.0;
		velY = 0.001;
