#pragma once
#ifndef PARTICLE_PLUS_H
#define PARTICLE_PLUS_H
#include "./../../Particle.h"
class Particle_Plus : public Particle {
private:
	float posX;
	float posY;
	float velX;
	...
	float lifetime;
	glm::vec4 color;
	glm::mat4x2 texturePos = TextureManager::getInstance().getTextureInfo(typeid(Particle_Plus));
...
public:
	...
	Particle_Plus(float posX, float posY, float scale, glm::vec4 tmpColor) : posX(posX), posY(posY), color(tmpColor) {
		velX = 0.0;
		velY = 0.001;
